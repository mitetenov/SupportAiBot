    ...
    async def _append(self, user_id: int, role: str, content: str) -> None:
        ...
        if self.db_manager is not None:
            try:
                ...
                logger.warning("Failed to persist chat message for user %d: %s", user_id, e)
        ...
        if self.db_manager is not None:
            try:
                async with self.db_manager.session() as session:
                    ...
                    logger.debug("Chat history deleted from DB for user %d", user_id)
            except Exception as e:
                logger.warning("Failed to delete chat history from DB for user %d: %s", user_id, e)
    ...
    async def evict_stale_entries(self, ttl_days: int | None = None) -> None:
        ...
        if self.db_manager is not None:
            try:
                ...
                logger.warning("Failed to evict stale chat messages: %s", e)
        ...
        for uid in stale_users:
            ...
            logger.debug("Evicted stale in-memory history for user %d", uid)
    ...
    async def _load_from_database(self, user_id: int) -> None:
        ...
        try:
            async with self.db_manager.session() as session:
                ...
                self._last_activity[user_id] = time.time()
                logger.debug(
                    "Loaded %d chat messages from DB for user %d",
                    len(self._histories[user_id]),
                    user_id,
                )
        except Exception as e:
            logger.warning("Failed to load chat history from DB for user %d: %s", user_id, e)
            self._histories[user_id] = deque(maxlen=self.max_messages)
            self._loaded_from_db.add(user_id)
