...
class SupportPrompt:
    ...
    SYSTEM: str = (
        ...
        "запрещено сразу выдавать простыню о смене способов оплаты и не направлять платёжные жалобы на общую диагностику VPN. "
        ...
        "первым профильным инструментом вызывай bedolaga_billing_get. Deposit без завершённой покупки периода означает, что "
        "нужно отдельно приобрести период из баланса в кабинете или боте; само по себе это состояние не является поводом для эскалации. "
