...
class BufferedMessage:
    ...
    #: The part of ``text`` the user actually wrote. None when ``text`` was
    #: generated by the bot — a captionless photo, where the prompt describing
    #: the screenshot is ours, not theirs.
...
class MessageBatch:
    ...
    base64_image: str | None = None
    mime_type: str | None = None
    #: Only the merged parts the user wrote themselves; empty when every part of
    #: ``text`` was generated by the bot. Anything reported back as the user's
    #: own question — knowledge gaps above all — has to read this, not ``text``.
    user_text: str = ""
    ...
    def of(cls, messages: list[BufferedMessage]) -> MessageBatch:
        ...
        authored_parts = [m.user_text for m in messages if m.user_text and m.user_text.strip()]
        merged_user_text = "\n".join(authored_parts)
