...
class TestContextualSearchQuery:
    ...
    def test_prefix_context_for_bare_platform_name(self, client: DeepSeekClient):
        ...
        assert client.build_contextual_search_query(USER_ID, "айпад") == f"{PREVIOUS} айпад"
        assert client.build_contextual_search_query(USER_ID, "ipad") == f"{PREVIOUS} ipad"
        assert client.build_contextual_search_query(USER_ID, "ios") == f"{PREVIOUS} ios"
        assert client.build_contextual_search_query(USER_ID, "ipados") == f"{PREVIOUS} ipados"
        ...
        assert client.build_contextual_search_query(USER_ID, "макос") == f"{PREVIOUS} макос"
        assert client.build_contextual_search_query(USER_ID, "macos") == f"{PREVIOUS} macos"
