...
- Фото из тикета появляется в топике. Фото оператора из топика загружается
  через Bedolaga и прикрепляется к ответу в тикете; подпись сохраняется.
...
- Для пересылки фото Bedolaga должен предоставлять сервисные API
  `POST /upload` и media-поля в `POST /tickets/{id}/reply`. Если отправка не
