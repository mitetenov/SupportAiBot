...
- Сборка кандидатов FAQ через производственный алгоритм `build_faq_context` без обращений к базе данных PostgreSQL и эмбеддингам, с поддержкой динамического исключения кандидатов (`exclude`).
- Изоляция сценарной памяти диалога (`ScenarioHistory`), корректная обработка форматов `role`/`content` и нативного формата Gemini (`user`/`model` с `parts`), а также отсутствие утечки состояния между сценариями и прогонами.
...
- Отклонение эвристическим scorer некорректных ответов и нарушений контракта: целых нерелевантных простыней FAQ, отсутствия уточняющих вопросов на неясную жалобу, повтора уже отвеченных вопросов из истории, лишних вызовов персональных инструментов для общего how-to, обратного порядка инструментов, ложных утверждений об отсутствии аккаунта при технических сбоях и ошибок эскалации (`/operator [ESCALATE]`).
...
- `require_clarification` и `max_questions` — требование задать 1–2 уточняющих вопроса при неясной жалобе.
