...
После успешного MCP tool call переключение намеренно останавливается при сбое
провайдера, чтобы не повторить потенциально изменяющее состояние действие. При
исчерпании всей цепочки (или этом безопасном останове) пользователь получает
