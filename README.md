...
## Синхронизация с топиком поддержки

Диалог пользователя и топик оператора держатся в согласованном состоянии:

- **Правки зеркалируются.** Пользователь отредактировал вопрос — обновляется копия в топике (текст или подпись к медиа), и в топик приходит пометка об этом. Оператор отредактировал ответ — обновляется сообщение у пользователя. Работает для сообщений, уместившихся в одно: правку нельзя отобразить на текст, разбитый на несколько сообщений.
- **Реакции пробрасываются** в обе стороны.
- **Недоставка видна.** Если сообщение оператора не дошло (пользователь заблокировал бота или удалил чат), в топик приходит явное «НЕ ДОСТАВЛЕНО» вместо подтверждения. Раньше сбой уходил только в логи. Такой ответ также не считается активностью оператора и не глушит AI на 30 минут.
