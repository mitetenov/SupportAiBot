...
- **Remnawave MCP** — состояние панели: подписка пользователя, узлы, HWID и
  персональная ссылка подписки, которую бот выдаёт только владельцу
  подтверждённого Telegram-аккаунта. Поднимается всегда.
